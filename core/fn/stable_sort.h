...
core t_any McoreFNstable_sort(t_thrd_data* const thrd_data, const t_any* const args) {
     const char* const owner = "function - core.stable_sort";
     ...
     case tid__obj: {
          ...
          t_any const result = obj__call__own(thrd_data, mtd__core_stable_sort, tid__obj, container, args, 2, owner);
     ...
     case tid__array: {
          call_stack__push(thrd_data, owner);
          t_any const result = array__stable_sort__own(thrd_data, container, is_asc_order.bytes[0] == 1, owner);
          call_stack__pop(thrd_data);
     ...
     case tid__custom: {
          call_stack__push(thrd_data, owner);
          t_any const result = custom__call__own(thrd_data, mtd__core_stable_sort, tid__custom, container, args, 2, owner);
          call_stack__pop(thrd_data);
