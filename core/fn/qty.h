...
core t_any McoreFNqty(t_thrd_data* const thrd_data, const t_any* const args) {
     const char* const owner = "function - core.qty";
     ...
     case tid__short_string:
          ...
          return short_string__qty(container, item);
     case tid__short_byte_array:
          if (item_type != tid__byte) [[clang::unlikely]] goto invalid_type_label;
          return short_byte_array__qty(container, item.bytes[0]);
     case tid__obj: {
          call_stack__push(thrd_data, owner);
          t_any const result = obj__qty__own(thrd_data, container, item, owner);
     ...
     case tid__table: {
          call_stack__push(thrd_data, owner);
          t_any const result = table__qty__own(thrd_data, container, item, owner);
          call_stack__pop(thrd_data);
     ...
     case tid__byte_array:
          if (item_type != tid__byte) [[clang::unlikely]] goto invalid_type_label;
          return long_byte_array__qty__own(thrd_data, container, item.bytes[0]);
     case tid__string:
          if (item_type != tid__char) [[clang::unlikely]] goto invalid_type_label;
          return long_string__qty__own(thrd_data, container, item);
     case tid__array: {
          call_stack__push(thrd_data, owner);
          t_any const result = array__qty__own(thrd_data, container, item, owner);
          call_stack__pop(thrd_data);
     ...
     case tid__custom: {
          call_stack__push(thrd_data, owner);
          t_any const result = custom__call__own(thrd_data, mtd__core_qty, tid__int, container, args, 2, owner);
          call_stack__pop(thrd_data);
