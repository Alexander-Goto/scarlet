...
core t_any McoreFN__div__(t_thrd_data* const thrd_data, const t_any* const args) {
     ...
     case tid__int: {
          ...
          if (right_i64 == 0 || (left_i64 == -9223372036854775808wb && right_i64 == -1)) [[clang::unlikely]] {
               ...
               return (const t_any){.structure = {.value = -9223372036854775808wb, .type = tid__int}};
          }
          return (const t_any){.structure = {.value = left_i64 / right_i64, .type = tid__int}};
     }
     case tid__float:
          return (const t_any){.structure = {.value = cast_double_to_u64(left.floats[0] / right.floats[0]), .type = tid__float}};
     case tid__obj: {
          call_stack__push(thrd_data, owner);
          t_any const result = obj__call__own(thrd_data, mtd__core___div__, tid__obj, left, args, 2, owner);
     ...
     case tid__custom: {
          call_stack__push(thrd_data, owner);
          t_any const result = custom__call__own(thrd_data, mtd__core___div__, tid__custom, left, args, 2, owner);
          call_stack__pop(thrd_data);
