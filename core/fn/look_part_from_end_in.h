...
[[gnu::hot]]
core t_any McoreFNlook_part_from_end_in(t_thrd_data* const thrd_data, const t_any* const args) {
     const char* const owner = "function - core.look_part_from_end_in";
     ...
     case tid__short_string:
          ...
          return look_part_from_end_in__short_string__short_string(array, part);
     case tid__short_byte_array:
          if (part_type == tid__byte_array) {
               ref_cnt__dec(thrd_data, part);
               return null;
          }
          return look_part_from_end_in__short_byte_array__short_byte_array(array, part);
     case tid__obj: {
          call_stack__push(thrd_data, owner);
          t_any result = obj__call__any_result__own(thrd_data, mtd__core_look_part_from_end_in, array, args, 2, owner);
     ...
     case tid__byte_array:
          return look_part_from_end_in__long_byte_array__byte_array__own(thrd_data, array, part);
     case tid__string:
          return look_part_from_end_in__long_string__string__own(thrd_data, array, part);
     case tid__array: {
          call_stack__push(thrd_data, owner);
          t_any const result = array__look_part_from_end_in__own(thrd_data, array, part, owner);
          call_stack__pop(thrd_data);
     ...
     case tid__custom: {
          call_stack__push(thrd_data, owner);
          t_any result = custom__call__any_result__own(thrd_data, mtd__core_look_part_from_end_in, array, args, 2, owner);
          if (result.bytes[15] != tid__int && result.bytes[15] != tid__null && result.bytes[15] != tid__error) [[clang::unlikely]] {
               ref_cnt__dec(thrd_data, result);
               result = error__invalid_type(thrd_data, owner);
          }
