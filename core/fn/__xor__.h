...
core t_any McoreFN__xor__(t_thrd_data* const thrd_data, const t_any* const args) {
     ...
     case tid__obj: {
          ...
          t_any const result = obj__call__own(thrd_data, mtd__core___xor__, tid__obj, left, args, 2, owner);
     ...
     case tid__custom: {
          call_stack__push(thrd_data, owner);
          t_any const result = custom__call__own(thrd_data, mtd__core___xor__, tid__custom, left, args, 2, owner);
          call_stack__pop(thrd_data);
