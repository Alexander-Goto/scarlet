...
static inline t_any great__own(t_thrd_data* const thrd_data, const t_any* const args, const char* const owner) {
     ...
     case tid__short_string:
          switch (right_type) {
          case tid__short_string:
               return cmp__short_string__short_string(left, right).bytes[0] == great_id ? bool__true : bool__false;
          case tid__string: {
               u64 const right_ref_cnt = get_ref_cnt(right);
               if (right_ref_cnt > 1) set_ref_cnt(right, right_ref_cnt - 1);

               t_any const result = cmp__short_string__long_string(left, right).bytes[0] == great_id ? bool__true : bool__false;
     ...
     case tid__string:
          ...
          case tid__short_string: {
               ...
               t_any const result = cmp__long_string__short_string(left, right).bytes[0] == great_id ? bool__true : bool__false;
          ...
          case tid__string: {
               ...
               t_any const result = cmp__long_string__long_string(left, right).bytes[0] == great_id ? bool__true : bool__false;
     ...
     case tid__int: case tid__time:
          return (i64)left.qwords[0] > (i64)right.qwords[0] ? bool__true : bool__false;
     case tid__float: {
     ...
     case tid__short_byte_array: case tid__byte_array: {
          ...
          t_any const result = byte_array__cmp(left, right).bytes[0] == great_id ? bool__true : bool__false;
...
core inline t_any McoreFN__great__(t_thrd_data* const thrd_data, const t_any* const args) {
     const char* const owner = "function - core.(>)";

     call_stack__push(thrd_data, owner);
     t_any const result = great__own(thrd_data, args, owner);
     call_stack__pop(thrd_data);
