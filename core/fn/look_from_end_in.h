...
[[gnu::hot]]
core t_any McoreFNlook_from_end_in(t_thrd_data* const thrd_data, const t_any* const args) {
     const char* const owner = "function - core.look_from_end_in";
     ...
     case tid__short_string:
          ...
          return short_string__look_from_end_in(array, item);
     case tid__short_byte_array:
          if (item_type != tid__byte) [[clang::unlikely]] goto invalid_type_label;
          return short_byte_array__look_from_end_in(array, item.bytes[0]);
     case tid__obj: {
          call_stack__push(thrd_data, owner);
          t_any result = obj__call__any_result__own(thrd_data, mtd__core_look_from_end_in, array, args, 2, owner);
     ...
     case tid__byte_array:
          if (item_type != tid__byte) [[clang::unlikely]] goto invalid_type_label;
          return long_byte_array__look_from_end_in__own(thrd_data, array, item.bytes[0]);
     case tid__string:
          if (item_type != tid__char) [[clang::unlikely]] goto invalid_type_label;
          return long_string__look_from_end_in__own(thrd_data, array, item);
     case tid__array: {
          call_stack__push(thrd_data, owner);
          t_any const result = array__look_from_end_in__own(thrd_data, array, item, owner);
          call_stack__pop(thrd_data);
     ...
     case tid__custom: {
          call_stack__push(thrd_data, owner);
          t_any result = custom__call__any_result__own(thrd_data, mtd__core_look_from_end_in, array, args, 2, owner);
          if (result.bytes[15] != tid__int && result.bytes[15] != tid__null && result.bytes[15] != tid__error) [[clang::unlikely]] {
               ref_cnt__dec(thrd_data, result);
               result = error__invalid_type(thrd_data, owner);
          }
